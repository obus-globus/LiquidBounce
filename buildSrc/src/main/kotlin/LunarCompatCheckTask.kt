...
abstract class LunarCompatCheckTask : DefaultTask() {
    ...
    fun run() {
        ...
        // Classes under injection/mixins/ without a `@Mixin` annotation (anonymous inner classes) parse to no
        // targets and are not mixins.
        val declared = mixins.filter { it.targets.isNotEmpty() }
        val referenced = declared.flatMap { it.targets }.toSet()
        ...
        // Coverage, so a partial bake cannot read as "0 broken, passed", and so the report states what it did
        // not look at rather than implying the whole client was covered.
        val expected = referenced.filter { internal -> BAKED_ROOTS.any(internal::startsWith) }
        val resolved = expected.count { targets[it] != null }
        val unmodelled = declared.filter { it.injectors.isEmpty() }
        val absent = declared.filter { mixin ->
            mixin.targets.none { internal -> BAKED_ROOTS.any(internal::startsWith) }
        }

        val coverage = buildString {
            append("Checked ${declared.size} mixin(s) (${declared.size - unmodelled.size} with injectors); ")
            append("$resolved/${expected.size} target classes resolved against Lunar's bake.")
            if (unmodelled.isNotEmpty()) {
                append("\nNot checked: ${unmodelled.size} mixin(s) declare no injector this task models, so only ")
                append("their target class is verified, not the members they shadow, overwrite or access: ")
                append(unmodelled.joinToString(", ") { it.name.substringAfterLast('/') })
            }
            if (absent.isNotEmpty()) {
                append("\nSkipped: ${absent.size} mixin(s) target classes Lunar does not ship, so nothing about ")
                append("them can be verified here: ")
                append(absent.joinToString(", ") { it.name.substringAfterLast('/') })
            }
        }

        if (expected.isNotEmpty() && resolved * 100 < expected.size * MIN_RESOLVED_PERCENT) {
            ...
            report.orNull?.asFile?.let {
                ...
                it.writeText("[ABORTED] $coverage\nToo few target classes resolved against Lunar's bake " +
                    "(empty/partial/failed bake); the findings would describe the bake, not the mixins.\n")
            }
            throw GradleException(
                "$coverage\nOnly $resolved of ${expected.size} target classes resolved - Lunar's baked classes " +
                    "look incomplete (a partial/failed bake), so the findings would describe the bake rather " +
                    "than the mixins. Aborting.",
            )
        }
    ...
    private companion object {
        ...
        // Package roots Lunar's bake ships. A target outside these belongs to a mod Lunar may not carry, so
        // its absence says nothing about compatibility.
        private val BAKED_ROOTS = listOf("net/minecraft/", "com/mojang/")

        // A bake this far short of resolving every expected target is broken, not evidence about the mixins.
        private const val MIN_RESOLVED_PERCENT = 95
    ...
    private fun checkMixin(mixin: MixinInfo, targets: Map<String, ClassNode>, findings: MutableList<Finding>) {
        // A target Lunar's bake should carry but does not means Lunar removed or renamed that class (or the
        // bake is incomplete), so the mixin cannot apply. Flag it once instead of silently skipping. Targets
        // outside those roots belong to mods Lunar may not ship and are legitimately absent.
        for (targetInternal in mixin.targets.toSet()) {
            if (targets[targetInternal] == null && BAKED_ROOTS.any(targetInternal::startsWith)) {
                findings += Finding(
