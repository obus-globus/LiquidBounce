...
./gradlew jar                                          # builds LiquidBounce (okhttp relocated for Lunar)
...
The static check engine is `buildSrc/src/main/kotlin/LunarCompatCheckTask.kt` (the same selector-matching engine used
for the NeoForge divergence check, pointed at Lunar's baked classes). The okhttp/okio relocation that lets LiquidBounce
run under Lunar at all is `buildSrc/src/main/kotlin/RelocateJarTask.kt`, wired into the `jar` task.
