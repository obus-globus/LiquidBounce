...
# If Lunar itself never finished init and LiquidBounce never launched, this is an environment/provisioning
# failure (download, API, headless), NOT a LiquidBounce incompatibility -- report it as such (exit 3).
if [ "$lunar_init" -eq 0 ] && [ "$lb_launched" -eq 0 ]; then
    echo "[runtime] INFRA: Lunar did not boot (environment/provisioning issue, not a LiquidBounce incompatibility)."
    exit 3
fi
